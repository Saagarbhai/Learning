...
class FlavorConfig {
  ...
  FlavorConfig._internal(
      {required this.flavor, required this.name, required this.env});

  static FlavorConfig get instance {
    if (_instance == null) {
      throw Exception(
          'FlavorConfig not initialized. Call FlavorConfig.initialize() before using it.');
    }
    return _instance!;
  }
  ...
  static void initialize({required Flavor flavor, required EnvConfig env}) {
    _instance ??=
        FlavorConfig._internal(flavor: flavor, name: flavor.name, env: env);
  }
